...
export const loginFunc = async (ctx, next) => {
  console.log(ctx.req.body);
  ctx.body = {code: 300, message: '登录故障，请联系管理员', data: {}};
  if (ctx.request.body.username && ctx.request.body.password) {
    try {
      let result = await User.findOne({username: ctx.request.body.username});
      let passport = passportFunc(ctx.request.body.password);
      ...
      if (result) {
        ...
        ctx.body = {code: 200, message: '你好：' + username + '，已自动为您注册账号', data: {
          passport, 
          userId: obj.value + 1, 
          username
        }};
    ...
    ctx.body = {code: 300, message: '用户名和密码请输入完整', data: {}};
  }
};
